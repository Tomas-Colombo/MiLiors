...
export function PuestoAcciones({ puestoId, activo }: Props) {
  ...
  // Modal de cierre/eliminación con registro de contratación
  ...
  // Cargar postulantes del puesto al abrir el modal (una sola vez)
  useEffect(() => {
    if (modo && postulantes === null) {
      getPostulantesDePuesto(puestoId).then(setPostulantes)
    }
  }, [modo, postulantes, puestoId])
  ...
  return (
    <div className="flex flex-col gap-2 items-center">
      ...
      {/* Errores fuera de modal (p. ej. reactivar) */}
      {error && modo === null && <Alert tone="error" title={error} className="text-xs" />}
      ...
      <Modal
        ...
        title={modo === 'eliminar' ? '¿Eliminar este puesto?' : '¿Cerrar este puesto?'}
        footer={
          <>
