...
export function NuevaEmpresaBtn() {
  ...
  return (
    <>
      ...
      {/* La lista se refresca sola por el revalidatePath de la action. */}
      <NuevaEmpresaModal open={abierto} onClose={() => setAbierto(false)} />
    </>
  )
}
...
export function EmpresaAcciones({ empresa }: { empresa: EmpresaDelReclutador }) {
  ...
  return (
    <>
      {/* Grilla fija y no flex: "Dar de baja" sólo existe si la empresa está
          activa, y con `justify-center` los otros dos se recentraban fila por
          fila. Con tres columnas cada botón cae siempre en el mismo lugar y el
          slot vacío simplemente queda vacío. */}
      <div className="grid grid-cols-3 items-center gap-1.5">
        <Link
          href={`/reclutador/postulaciones?empresa=${empresa.id}`}
          className={`${accion} border border-neutral-300 bg-surface text-ink-soft hover:bg-neutral-50`}
        >
          Ver postulaciones
        </Link>
        <button
        ...
        {empresa.activa && (
          <button
            ...
            className={`${accion} bg-error-solid text-white hover:bg-error-strong`}
          >
            Dar de baja
          </button>
        )}
