...
export const getTestActual = cache(async () => {
  ...
  // Obtener test vigente
  const { data: test } = await supabase
    .from('test_eneagrama')
    .select('id, fecha_realizacion, eneatipo_id, updated_at')
  ...
  return {
    ...
    test: test as { id: string; fecha_realizacion: string; eneatipo_id: string | null; updated_at: string },
